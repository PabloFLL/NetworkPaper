...
def generar_grupos_aleatorios_unicos(nodes, k, n_iter, seed=42):
    """
    Genera grupos aleatorios únicos de tamaño k.
    Evita repetir la misma combinación de nodos.
    """
    random.seed(seed)
    ...
    max_intentos = n_iter * 50
    intentos = 0

    while len(grupos) < n_iter and intentos < max_intentos:
        grupo = tuple(sorted(random.sample(nodes, k)))
        grupos.add(grupo)
        intentos += 1

    if len(grupos) < n_iter:
        print(f"Aviso: solo se han generado {len(grupos)} grupos únicos de {n_iter} solicitados.")

    return [set(g) for g in grupos]
...
def randomization_test_proporcion_q1(
    G,
    nodos_transversales,
    metrica="degree",
    n_iter=10000,
    ...
    """
    Test de aleatorización sin grupos repetidos.
    Comprueba si hay más transversales en Q1 de lo esperable al azar.
    """
    ...
    grupos_random = generar_grupos_aleatorios_unicos(
        nodes,
        k_trans,
        n_iter,
        seed=seed
    ...
    random_counts = np.empty(len(grupos_random), dtype=int)

    for i, random_trans in enumerate(grupos_random):
        random_counts[i] = len(q1_nodes & random_trans)

    random_props = random_counts / q1_size

    if alternative == "greater":
        p_value = (np.sum(random_counts >= obs_count) + 1) / (len(random_counts) + 1)
    elif alternative == "two-sided":
        mean_null = random_props.mean()
        p_value = (
            np.sum(np.abs(random_props - mean_null) >= abs(obs_prop - mean_null)) + 1
        ) / (len(random_props) + 1)
    else:
        raise ValueError("alternative debe ser 'greater' o 'two-sided'.")

    print(f"\n=== RANDOMIZATION TEST Q1 ({metrica.upper()}) ===")
    ...
    print("IC95% nulo:", (
        float(np.percentile(random_props, 2.5)),
        float(np.percentile(random_props, 97.5))
    ))
    print("Iteraciones únicas:", len(grupos_random))
...
def randomization_test_betweenness_global(
    G,
    nodos_transversales,
    n_iter=10000,
    seed=42,
    ...
    """
    Test de aleatorización sin grupos repetidos.
    Comprueba si la betweenness media de los transversales
    es mayor que la de los disciplinares.
    """
    ...
    diff_obs = float(np.mean(trans_real) - np.mean(disc_real))
    ...
    diffs_null = np.empty(len(grupos_random), dtype=float)
    ...
    if alternative == "greater":
        ...
        p_value = (
            np.sum(np.abs(diffs_null - mean_null) >= abs(diff_obs - mean_null)) + 1
        ) / (len(diffs_null) + 1)
    else:
        raise ValueError("alternative debe ser 'greater' o 'two-sided'.")

    print("\n=== RANDOMIZATION TEST BETWEENNESS GLOBAL ===")
    ...
    print("Diferencia observada:", diff_obs)
    print("Media nula:", float(diffs_null.mean()))
    print("IC95% nulo:", (
        float(np.percentile(diffs_null, 2.5)),
...
# ============================================================
# RANDOMIZATION TEST 3:
# DIFERENCIA DE BETWEENNESS DENTRO DEL TOP 25%
# ============================================================

def randomization_test_betweenness_top_q1(
    G,
    nodos_transversales,
    metrica="degree",
    n_iter=10000,
    ...
    """
    Test de aleatorización sin grupos repetidos.
    Mantiene fijo el Q1 y randomiza qué nodos son transversales.
    """
    ...
    if len(q1_trans_real) == 0 or len(q1_disc_real) == 0:
        print(f"\n=== RANDOMIZATION TEST BETWEENNESS TOP Q1 ({metrica.upper()}) ===")
    ...
    for random_trans in grupos_random:
    ...
    if len(diffs_null) == 0:
        print("No hay aleatorizaciones válidas.")
        return None
    ...
    print(f"\n=== RANDOMIZATION TEST BETWEENNESS TOP Q1 ({metrica.upper()}) ===")
    print("Iteraciones únicas generadas:", len(grupos_random))
...
def main():
    ...
    # Usamos la red no dirigida simple para centralidad estructural.
    G = Gtotalsimple
    ...
    # ========================================================
    # RANDOMIZATION TESTS SIN REPETIR GRUPOS
    # ========================================================

    randomization_test_proporcion_q1(
        G,
        nodos_transversales_presentes,
        metrica="degree",
        n_iter=10000,
    ...
    randomization_test_betweenness_global(
        G,
        nodos_transversales_presentes,
        n_iter=10000,
        seed=42,
    ...
    randomization_test_betweenness_top_q1(
        G,
        nodos_transversales_presentes,
        metrica="degree",
        n_iter=10000,

...
def main():
    ...
    # -------------------------
    ciclo = 0
    while not nx.is_directed_acyclic_graph(grafototal):
        ciclo += 1

        try:
            cycle = nx.find_cycle(grafototal, orientation='original')
        ...
        if cycle:
            ...
            u, v, *_ = cycle[0]
            if grafototal.has_edge(u, v):
                grafototal.remove_edge(u, v)
            else:
    ...
    # ----------------------------------------------------------------
    grados_dag = dict(grafototal.degree())  # grado total (in-degree + out-degree)
    ...
    # ----------------------------------------------------------------
    # En este punto, los nodos de grafototal siguen siendo los nombres originales.
    ...
    grafototal = nx.relabel_nodes(grafototal, diccionario_ids)

    print("Nodos con nuevos IDs:", list(grafototal.nodes))
    print(f"Is this graph a DAG? {nx.is_directed_acyclic_graph(grafototal)}")
    topological_order = list(nx.topological_sort(grafototal))
    print(f"Topological Order: {topological_order}")

    def contar_caminos_en_DAG(G):
        """
        Cuenta el número total de caminos dirigidos en un DAG.
        Incluye caminos triviales de longitud 0 (un solo nodo).
        Si quieres solo caminos de longitud ≥ 1, luego restas el número de nodos.
        """

        # Comprobación de seguridad: si no es DAG, mejor avisar
        if not nx.is_directed_acyclic_graph(G):
            raise ValueError("El grafo no es un DAG; todavía quedan ciclos.")

        # Orden topológico de los nodos
        topo = list(nx.topological_sort(G))

        # paths_from[u] = número de caminos que empiezan en u
        # empezamos asumiendo 1 camino trivial: u solo
        paths_from = {u: 1 for u in topo}

        # Recorremos en orden topológico inverso
        for u in reversed(topo):
            total = 1  # el camino trivial [u]
            for v in G.successors(u):
                total += paths_from[v]
            paths_from[u] = total

        # Número total de caminos (sumando los que empiezan en cada nodo)
        total_paths_including_trivial = sum(paths_from.values())

        # Si quieres solo caminos de longitud ≥ 1:
        total_nontrivial_paths = total_paths_including_trivial - G.number_of_nodes()

        return total_paths_including_trivial, total_nontrivial_paths, paths_from

    total_paths_inc_trivial, total_paths_no_trivial, paths_from_node = contar_caminos_en_DAG(grafototal)

    print("\n=== CAMINOS ACÍCLICOS EN EL DAG ===")
    print(f"Caminos totales (incluyendo caminos triviales de longitud 0): {total_paths_inc_trivial}")
    print(f"Caminos no triviales (longitud ≥ 1): {total_paths_no_trivial}")

    # ----------------------------------------------------------------
    # 5) Medida básico-aplicado (sobre el DAG con IDs)
    # ----------------------------------------------------------------
    medida_basico_aplicado = {
        node: grafototal.out_degree(node) - grafototal.in_degree(node)
        for node in grafototal.nodes
    }
    ...
    colores_nodos = [cmap(norm(medida_basico_aplicado[n])) for n in grafototal.nodes]
    ...
    # pos = graphviz_layout(grafototal, prog='dot')
    #
    # # 1) Dibujar todos los nodos con color según básico-aplicado
    # nx.draw(
    #     grafototal, pos, with_labels=True, ax=ax,
    ...
    #         grafototal, pos, nodelist=nodos_destacados_ids, ax=ax,
    ...
    print("The number of edges in DAG is:", nx.number_of_edges(grafototal))

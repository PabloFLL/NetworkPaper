...
def construir_curr_pos_desde_cronologia(G_base: nx.DiGraph):
    """
    Construye la correspondencia currículum → nodos de tu red
    e imprime diagnóstico de emparejamiento.
    """

    # ------------------------------------------------------------
    # 1) DEFINICIÓN DE BLOQUES (currículum)
    # ------------------------------------------------------------
    bloques = {
        1: [
            "Matrices y determinantes: clasificación, propiedades, operaciones y aplicaciones (grafos y modelización de situaciones reales).",
            "Demostraciones numéricas sencillas (inducción, deducción…).",
        ...
        2: [
            "Resolución de problemas mediante ecuaciones e inecuaciones.",
            "Interpretación gráfica de las soluciones de ecuaciones, inecuaciones y sistemas con y sin medios tecnológicos.",
            "Método de Gauss.",
            "Discusión y resolución de sistemas de ecuaciones lineales. Regla de Cramer.",
            "Resolución de problemas algebraicos mediante matrices y determinantes.",
        ...
        3: [
            "Funciones básicas: polinómica, racional e irracional, definidas a tramos, exponencial, logarítmica, trigonométrica, periódica, valor absoluto. Características necesarias para la construcción gráfica.",
            "Continuidad y discontinuidad. Asíntotas y ramas.",
            "Estimación de límites mediante tablas o gráficas. Cálculo de límites en un punto y en el infinito. Indeterminaciones. Infinitos e infinitésimos. Regla de L’Hôpital.",
            ...
            "Reglas y técnicas de derivación. Cálculo de derivadas.",
            "Derivabilidad de una función. Teoremas de Rolle y del valor medio.",
            "Uso de la derivada en contextos STEM: representación gráfica, estudio del cambio y optimización.",
            "Primitiva de una función. Integrales inmediatas y técnicas para el cálculo de primitivas (resolución por partes y sustitución).",
            "Integral definida. Propiedades. Teorema fundamental del cálculo integral.",
            "Regla de Barrow. Área de superficies planas y volúmenes de revolución.",
        ...
        4: [
            ...
            "Vectores libres en el plano. Operaciones geométricas básicas y sus propiedades. Producto escalar, vectorial y mixto.",
            "Dependencia e independencia lineal. Bases ortogonales y ortonormales. Sistemas de referencia, coordenadas de un vector respecto de una base.",
            "Representación de objetos geométricos en el plano con y sin medios tecnológicos.",
            "Ecuaciones rectas y planos en el espacio. Posiciones relativas. Problemas métricos en el espacio: distancias, ángulos, superficies y volúmenes.",
            "Desarrollo histórico de la geometría analítica y sus aplicaciones. Valoración de los usos en contextos científicos.",
        ],
        5: [
            "Experimentos aleatorios y sucesos. Frecuencias e idea intuitiva de probabilidad. Sucesos. Dependencia e independencia de sucesos.",
            "Estrategias de recuento para el cálculo de probabilidades. Diagramas de árbol y tablas de contingencia. Regla de Laplace.",
            "Probabilidad condicionada. Teoremas: probabilidad total y Bayes.",
            "Modelización de fenómenos estocásticos mediante distribuciones binomial y normal. Utilización de herramientas tecnológicas para el cálculo de probabilidades cuando sea necesario.",
        ...
        6: [
            ...
            "Demostraciones sencillas (métodos de reducción al absurdo, inducción completa, razonamiento deductivo…).",
    ...
    # ------------------------------------------------------------
    # 2) EMPAREJAMIENTO Y DIAGNÓSTICO
    # ------------------------------------------------------------
    faltantes = []
    asignaciones = []  # (bloque, k, item, nodo_emparejado)

    existing_nodes = list(G_base.nodes())
    curr_pos = {}

    total_items = sum(len(v) for v in bloques.values())
    ...
    for b in sorted(bloques.keys()):
        for k, item in enumerate(bloques[b]):
            ...
            if nodo is None:
                ...
                continue
            asignaciones.append((b, k, item, nodo))
            curr_pos.setdefault(nodo, (b, k))

    # detectar colisiones: varios ítems → mismo nodo
    inv = defaultdict(list)
    for b, k, item, nodo in asignaciones:
        inv[nodo].append((b, k, item))

    colisiones = {nodo: items for nodo, items in inv.items() if len(items) > 1}

    print("\n=== DIAGNÓSTICO CURRÍCULUM ===")
    print(f"Ítems totales en cronología: {total_items}")
    print(f"Ítems NO emparejados: {len(faltantes)}")
    print(f"Nodos únicos emparejados (curr_pos): {len(curr_pos)}")
    print(f"Colisiones (2+ ítems → mismo nodo): {len(colisiones)}")

    if faltantes:
        print("\nÍtems NO emparejados:")
        for b, k, item in faltantes:
            print(f" - Bloque {b}, pos {k}: {item}")

    if colisiones:
        print("\nColisiones (mismo nodo de tu red para varios ítems):")
        for nodo, items in list(colisiones.items())[:20]:
            print(f"\nNodo en tu red: {nodo}")
            for (b, k, item) in items:
                print(f"   - Bloque {b}, pos {k}: {item}")
...
def main():
    ...
    ruta_excel = r"C:\Users\pablo\Desktop\PAPERS\Redes\matesidentificador.xlsx"
